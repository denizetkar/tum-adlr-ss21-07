...
nohup sh -c 'python main.py train --ppo-model-path "./checkpoints/recurrent-ppo-no-curiosity.zip" --tensorboard-log "./tensorboard/MountainCarContinuous-v0_no_curiosity" --device cuda --total-timesteps 5000000 --rnn-hidden-dim 32 --policy RnnPolicy --env "MountainCarContinuous-v0"' &> nohup-no-curiosity.out &
